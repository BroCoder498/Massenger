...
app.get('/api/messages', authMiddleware, async (req: any, res) => {
  ...
  const withUsers = await Promise.all(msgs.map(async (m) => {
    const [sender, recipient] = await Promise.all([
      prisma.user.findUnique({ where: { id: m.senderId }, select: { username: true } }),
      prisma.user.findUnique({ where: { id: m.recipientId }, select: { username: true } }),
    ]);
    return {
      id: m.id,
      chatId: m.chatId,
